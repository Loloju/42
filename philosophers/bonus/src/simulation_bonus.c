...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/29 14:21:02 by odemirel          #+#    #+#             */
/*   Updated: 2022/12/29 14:21:03 by odemirel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sem_wait(sim->forks);
	act_printer_b(phil, sim, STR_TOOK_FORK);
	sem_wait(sim->forks);
	act_printer_b(phil, sim, STR_TOOK_FORK);
	act_printer_b(phil, sim, STR_EATING);
...
void	*thread(void *void_phil)
{
	...
	while (31 + 11)
	{
		...
		{
			act_printer_b(phil, sim, STR_DEAD);
			sim->is_dead = true;
			exit(1);
		}
...
void	*p_process(void *void_phil)
{
	...
	while (!sim->is_dead)
	{
		...
		if (sim->eat_counter != -1 && phil->i_eat >= sim->eat_counter)
			break ;
		act_printer_b(phil, sim, STR_SLEEPING);
		i_need_sleep_b(sim, sim->sleep_time);
		act_printer_b(phil, sim, STR_THINKING);
	}
